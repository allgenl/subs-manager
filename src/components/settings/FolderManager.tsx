...
import { Folder } from '@/types/subscription';
          ...
          {folders.map((f) => (
            <div key={f.id} className="flex items-center justify-between rounded-lg bg-gray-50 px-3 py-2 dark:bg-gray-800">
              <div className="flex items-center gap-2">
                <div className="h-3 w-3 rounded" style={{ backgroundColor: f.color }} />
        ...
        <div className="flex gap-1 items-end pb-1">
          {FOLDER_COLORS.map((c) => (
            <button
              ...
              type="button"
              onClick={() => setColor(c)}
              className={`h-6 w-6 rounded border-2 ${color === c ? 'border-gray-900 dark:border-white' : 'border-transparent'}`}
              style={{ backgroundColor: c }}
            />
          ))}
        </div>
        <Button size="sm" onPress={handleAdd} className="shrink-0 self-end">
          <Plus size={14} />
        </Button>
      </div>
    </Card>
